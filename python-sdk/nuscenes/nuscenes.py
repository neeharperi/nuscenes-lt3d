...
class NuScenesExplorer:
    ...
    def map_pointcloud_to_image_new_pts(self,
        ...
        # Take the actual picture (matrix multiplication with camera-matrix + renormalization).
        orig_points = view_points(pc.points[:3, :], np.array(cs_record['camera_intrinsic']), normalize=True)
        # import ipdb; ipdb.set_trace()
        # Remove points that are either outside or behind the camera. Leave a margin of 1 pixel for aesthetic reasons.
        # Also make sure points are at least 1m in front of the camera to avoid seeing the lidar points on the camera
        # casing for non-keyframes which are slightly out of sync.
        mask = np.ones(depths.shape[0], dtype=bool)
        mask = np.logical_and(mask, depths > min_dist)
        mask = np.logical_and(mask, orig_points[0, :] > 1)
        mask = np.logical_and(mask, orig_points[0, :] < im.size[0] - 1)
        mask = np.logical_and(mask, orig_points[1, :] > 1)
        mask = np.logical_and(mask, orig_points[1, :] < im.size[1] - 1)
        points = orig_points[:, mask]
        coloring = coloring[mask]

        return points, coloring, im, orig_points, mask
    ...
    def render_pointcloud_in_image_new_pts(self,
        ...
        if display_viz:
            if ax is None:
                ...
                ax.set_title(camera_channel)
            ax.imshow(im)
            ax.scatter(points[0, :], points[1, :], c=coloring, s=dot_size*3)
            ax.axis('off')
