...
def class_pr_curve(md_list: DetectionMetricDataList,
    ...
    for md, dist_th, semantic_th in data:
        ...
        ax.plot(md.recall, md.precision, label='Dist. : {}, Semantic: {}, AP: {:.1f}'.format(dist_th, dist_th, ap * 100))
...
def class_tp_curve(md_list: DetectionMetricDataList,
    ...
    md = md_list[(detection_name, dist_th_tp, 0)]
...
def dist_pr_curve(md_list: DetectionMetricDataList,
    ...
    data = md_list.get_dist_data(dist_th, semantic_th)
    for md, detection_name in data:
        md = md_list[(detection_name, dist_th, semantic_th)]
        ap = metrics.get_label_ap(detection_name, dist_th, semantic_th)
